    ...
    <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex justify-center items-center">
      <div className="w-full h-full bg-black text-white overflow-hidden">
        ...
        <div className="py-5 flex justify-center items-center gap-10 md:px-10 lg:px-32">
          <div className="w-4/5 md:h-[30vh] lg:h-[75vh] relative">
            {isPlaying ? (
          ...
          <div className="w-1/5 md:max-h-[30vh] lg:max-h-[75vh] overflow-y-auto">
        ...
        {/* Other Video */}
        <div className="absolute bottom-0 left-0 w-full p-4 bg-black bg-opacity-80 overflow-y-hidden">
          <div className="flex space-x-4 overflow-x-auto overflow-y-hidden">
            {items.map((item) => (
              <div key={item.id} onClick={() => setActiveItem(item)} className={`relative flex-shrink-0 w-40 h-20 cursor-pointer transition-transform duration-300 ${activeItem.id === item.id ? "transform scale-110" : ""}`}>
